...
Ptr<REQ> CreateDspReq01(bool repeat, u16 tryCount)
{
	...
	rq->rsp = NandFlash_AllocWB(sizeof(RspDsp30::Hdr)+mv.sampleLen*2+2);
...
{
	Ptr<MB> rq(NandFlash_AllocWB(sizeof(RspDsp30::h) + 1024*2 + 2));
...
static bool RequestFlashWrite_00()
{
	...
	if (mb.Valid())
	{
		...
		return NandFlash_RequestWrite(mb, data[0], true);
	};
...
//	static TM32 tm;
	...
	switch (mainModeState)
	{
		...
		case 2:
		{
			NandFlash_RequestWrite(rq->rsp, rsp->h.rw, false);
...
//	if (count != 0)
	{
		...
		if (ptr.Valid())
		{
			...
			NandFlash_RequestWrite(ptr, rsp->rw, true);
...
static void FlashDSP_Direct()
{
	...
	if (flashCRC != fcrc || flashLen != flen)
	{
		...
		if (at25df021_Write((byte*)fpages, 0, flen, true))
		{
