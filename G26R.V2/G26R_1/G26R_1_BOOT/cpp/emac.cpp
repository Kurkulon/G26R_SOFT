...
static void ReqReadPHY(byte PhyReg);
	...
	inline bool CheckBufferOwn(Buf_Desc &buf) { return (u32)buf.addr1 & OWNERSHIP_BIT; }
	inline void SetBufferOwn(Buf_Desc &buf) { *((u32*)&buf.addr1) &= ~OWNERSHIP_BIT; }
	inline byte* GetBufferAdr(Buf_Desc &buf) { return (byte*)((u32)buf.addr1 & ~(OWNERSHIP_BIT|WRAP_BIT)); }
	inline void SetBufferAdr(Buf_Desc &buf, void *adr) { buf.addr1 = (void*)(((u32)buf.addr1 & (OWNERSHIP_BIT|WRAP_BIT)) | ((u32)adr & ~(OWNERSHIP_BIT|WRAP_BIT))); }
	void ResetPHY() { 	HW::PIOC->CLR((1<<15)); };
	void EnablePHY() { 	HW::PIOC->SET((1<<15)); };
#elif defined(CPU_XMC48)
	inline void EnableMDI() { /*HW::GMAC->NCR |= GMAC_MPE;*/ }
	inline void DisableMDI() { /*HW::GMAC->NCR &= ~GMAC_MPE;*/ }
	...
	inline byte* GetBufferAdr(Receive_Desc &buf) { return (byte*)buf.addr1; }
	inline void SetBufferAdr(Receive_Desc &buf, void *adr) { buf.addr1 = adr; }
...
static void FreeTxDesc()
{
	...
	if ((td.stat & TD_TRANSMIT_OK) && (td.stat & TD_LENGTH_MASK) != 0)
	{
		...
		EthBuf* b = (EthBuf*)((u32)td.addr1 - 8);
...
static void RecieveFrame()
{
	...
	else
	{
		...
		if (buf.addr1 != 0 && CheckRecievedFrame(buf)) // buffer contains a whole frame
		{
			byte *b = GetBufferAdr(buf);

			mb = (MEMB*)(b - sizeof(mb->next));
		...
		if (GetBufferAdr(buf) == 0)
		{
			MEMB* nmb = AllocMemBuffer();

			if (nmb != 0)
			{
				SetBufferAdr(buf, nmb->data);
				SetBufferOwn(buf);
			};
		}
		else
...
static void rx_descr_init (void)
{
	...
	for (u32 i = 0; i < NUM_RX_BUF; i++)
	{
		...
		*((u32*)(&Rx_Desc[NUM_RX_BUF-1].addr1)) |= WRAP_BIT; // Set the WRAP bit at the end of the list descriptor.
...
static void tx_descr_init (void)
{
	TxBufIndex = 0;
		...
		{
			Tx_Desc[i].addr1 = 0;//(u32)&tx_buf[i];
